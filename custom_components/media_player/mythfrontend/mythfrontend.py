...
class MythTVFrontendDevice(MediaPlayerDevice):
    ...
    def __init__(self, host_frontend, port_frontend, host_backend,
        ...
        self._api_frontend = self._api.Send(host=self._host_frontend,
                                  port=self._port_frontend)

        self._api_backend = self._api.Send(host=self._host_backend,
    ...
    def api_update(self):
        ...
        try:
            result = self._api_frontend.send(endpoint='Frontend/GetStatus',
                                   opts={'timeout': 1})
    ...
    def _get_artwork(self):
        ...
        result = self._api_backend.send(endpoint=endpoint,
                                         opts={'timeout': 2})
        if list(result.keys())[0] in ['Abort', 'Warning']:
    ...
    def api_send_action(self, action, value=None):
        ...
        try:
            result = self._api_frontend.send(endpoint='Frontend/SendAction',
